...
export async function initialize(manager, invidious) { try { await manager.refresh(); await manager.healthCheckAll(); } catch (error) { log(`[WARN] Initial Piped instance discovery failed: ${error.message}`); } try { await invidious.healthCheckAll(); } catch (error) { log(`[WARN] Initial Invidious health check failed: ${error.message}`); } }
...
async function main() {
  const config = loadConfig(); const { server, manager, invidious } = createApp(config); await initialize(manager, invidious); const tasks = [refreshLoop(manager, invidious, config), healthLoop(manager, invidious, config)];
